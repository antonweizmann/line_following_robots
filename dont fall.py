...
def level3_sort_and_compact(
    bumper:    tuple, # Type hint broadened to handle unexpected CoppeliaSim returns
    top_img:   np.ndarray,
    small_img: np.ndarray,
) -> tuple[float, float] | None:
    global BLOB_FOUND
    ...
    green_found, _, _ = detect_blob(small_img, "green")
    brown_found, _, _ = detect_blob(small_img, "brown")
    black_found, _, _ = detect_blob(small_img, "black")
...
def level2_seek_object(top_img: np.ndarray, small_img: np.ndarray) -> tuple[float, float] | None:
    ...
    # black_found, x_black, _ = detect_blob(top_img[(int(len(top_img) / 2)): ], "black")
...
def level0_survive(sonar: float, cmd: tuple[float, float], top_img: np.ndarray) -> tuple[float, float]:
    blue_found, x, _ = detect_blob(top_img, 'blue')
    red_found, x, _ = detect_blob(top_img, "red")
    obstacle_present = (sonar >= 0) and (sonar < SONAR_DANGER_DIST) and not BLOB_FOUND and (blue_found or red_found)
    if not obstacle_present:
        return cmd
    print("=====Avoid=====")
    print(f"Sonar : {sonar}, BLOB: {BLOB_FOUND}, Blue Bin: {blue_found}, Red Bin: {red_found}")
    return (-BASE_SPEED * 0.8, -BASE_SPEED * 1.2)
