...
#  Tunable Constants
# ─────────────────────────────────────────────────────────────────────────────
BATTERY_LOW_THRESHOLD  = 10.0   # % — triggers Recharge
BUMPER_FORCE_THRESHOLD = 0.5    # N — triggers Sort_and_Compact
SONAR_DANGER_DIST      = 0.2    # m — triggers Survive (−1 means no reading)
...
_COLOUR_RANGES: dict[str, dict[str, tuple[int, int]]] = {
    ...
    "brown":  {"r": ( 80, 200), "g": ( 40, 130), "b": (  0,  60)},
...
def level3_sort_and_compact(
    ...
    """
    if max(bumper) < BUMPER_FORCE_THRESHOLD:
        return None
...
def level0_survive(
    ...
    obstacle_present = (sonar >= 0) and (sonar < SONAR_DANGER_DIST)
